...
// Simple Image component for React (not Next.js)
const Image = ({src, alt, className, onLoad, loading = "lazy", ...rest}: ImageProps) => {
	return (
		<img
			src={src}
			alt={alt}
		...
		<>
				...
				{isExpanded && (
					<div className="fixed inset-0 h-screen overflow-hidden z-50">
						<motion.div
							...
							animate={{opacity: 1}}
							exit={{opacity: 0}}
							className="bg-white/80 backdrop-blur-lg h-full w-full fixed inset-0"
						/>
						<motion.div
							initial={{opacity: 0}}
							animate={{opacity: 1}}
							...
							className="max-w-5xl mx-auto bg-gradient-to-b from-background to-background/95 h-full z-[60] p-4 md:p-10 rounded-3xl relative md:mt-10 border"
						>
							<button
							...
							<motion.p
								...
								className="px-0 md:px-20 text-2xl md:text-4xl font-normal italic text-foreground mt-4 lowercase"
							>
								{testimonial.name}
							</motion.p>
							<div className="py-8 text-foreground px-0 md:px-20 text-3xl lowercase font-thin leading-snug tracking-wide">
			...
			>
				...
				>
					...
					<motion.p
						layoutId={layout ? `title-${testimonial.name}` : undefined}
						className="text-foreground text-2xl md:text-2xl font-normal text-center [text-wrap:balance] mt-4 lowercase px-3"
					>
					...
					<motion.p
						layoutId={layout ? `category-${testimonial.name}` : undefined}
						className="text-foreground text-xl md:text-2xl font-thin italic text-center mt-5 lowercase"
					>
						{testimonial.name}.
					</motion.p>
					<motion.p
						layoutId={layout ? `category-${testimonial.name}` : undefined}
						className="text-muted-foreground text-base md:text-base font-thin italic text-center mt-1 lowercase underline underline-offset-8 decoration-1"
					>
...
const ProfileImage = ({src, alt, ...rest}: ImageProps) => {
	...
	return (
		<div className="w-[90px] h-[90px] md:w-[150px] md:h-[150px] opacity-80 overflow-hidden rounded-full border-[3px] border-solid border-border aspect-[1/1] flex-none saturate-[0.2] sepia-[0.46] relative">
			<Image
				className={cn(
