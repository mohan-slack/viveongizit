...
export default function EnhancedHeroSection({
  ...
  return <section id="hero" className="hero hero-fullbleed relative h-[100dvh] w-full overflow-hidden">
      {/* ---------- Video Background ---------- */}
      {showVideo && <div className={`absolute inset-0 w-full h-full z-0 transition-opacity duration-700 ${videoLoaded ? 'opacity-100' : 'opacity-0'}`}>
          <video ref={videoRef} className="w-full h-full object-cover object-center" autoPlay muted loop playsInline poster={videoPoster || heroBlackGlossy} onLoadedData={handleVideoLoaded} onError={handleVideoError}>
      ...
      align: "start"
          ...
          <CarouselContent className="h-full -ml-0">
            {heroImages.map((image, index) => <CarouselItem key={index} className="relative h-full w-full pl-0 basis-full">
                ...
                <div className="relative w-full h-full z-0">
                  <picture className="w-full h-full">
                    ...
                    <img src={image.src} alt={image.alt} className="w-full h-full object-cover object-center block" loading={index === 0 ? "eager" : "lazy"} />
                  </picture>
      ...
      <div className="hero-overlay">
        <div className="hero-content">
          ...
          <img alt="HUX Logo" className="hux-logo fade-in object-cover rounded-xl" src="/lovable-uploads/f866d3b7-d8bf-4fa5-a6de-96bab5fedc3f.png" />
